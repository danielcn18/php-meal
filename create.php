    ...
    <form action="" method="post" style="width: 80%; margin: 2rem 10% 0 10%;">
        ...
        <div class="card d-flex flex-row w-full mb-4" style="padding: 1rem; border-radius: 1rem;"> 
            ...
            <div class="card-body"> 
                ...
                <input style="height: 2.2rem;" type="number" name="quantity[]" value="1" min="1"> 
                <button type="button" class="btn btn-primary bg-primary" style="background-image: linear-gradient(to right, #08b2e3, #7c98b3); color: #fff; border-color: #7ee8fa;">
                    Add To Cart
                </button> 
            </div> 
        </div>
...
<?php
    ...
    if (isset($_POST['create']) && !empty($_POST['create'])) {
        ...
        if (
            ...
            $query = "INSERT INTO orders(firstname, lastname, phone, streetaddress, streetaddress2, city, zip, items) 
                    VALUES('{$fname}', '{$lname}', '{$phone}', '{$straddr}', '{$straddr2}', '{$city}', '{$zip}', '{$itemsJson}')";
